...
async function compilePlugin(zipPath, buildId) {
    ...
    try {
        ...
        if (!hasGradle && !hasMaven) {
            throw new Error('No build.gradle or pom.xml found!');
        }
        ...
        if (hasGradle) {
            ...
            // Use system gradle with memory limits
            // -Xmx256m = max 256MB heap
            // -XX:MaxMetaspaceSize=128m = max 128MB metaspace
            ...
            // === MAVEN BUILD - MEMORY OPTIMIZED ===
            buildType = 'Maven';
            command = 'mvn clean package';
            buildOutputPath = path.join(projectRoot, 'target');
        }

        // 4. Run build with memory limits
        ...
        const output = execSync(command, {
            ...
            env: { 
                ...
                GRADLE_OPTS: '-Dorg.gradle.daemon=false -Dorg.gradle.configureondemand=true -Dorg.gradle.jvmargs=-Xmx256m -XX:MaxMetaspaceSize=128m -XX:+UseSerialGC',
                MAVEN_OPTS: '-Xmx256m -XX:MaxMetaspaceSize=128m -XX:+UseSerialGC',
                _JAVA_OPTIONS: '-Xmx256m -XX:MaxMetaspaceSize=128m -XX:+UseSerialGC'
            }
        });
